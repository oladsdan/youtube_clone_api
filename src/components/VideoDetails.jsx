...
const VideoDetails = () => {
  ...
  useEffect(()=>{
    fetchFromApis(`videos?part=snippet,statistics&id=${id}`)
     ...
     //we make a second api for related videos
     fetchFromApis(`search?part=snippet&relatedToVideoId=${id}&type=video`)
      .then((data) => setVideosRelated(data.items)) 

  },[id]);

  if (!videoDetail?.snippet && !videosRelated) return 'Loading....';
  ...
  return (
    <Box minHeight="95vh">
      ...
      <Box px={2} py={{md: 1, xs:5}} justifyContent="center" alignItems="center">
        <VideosDisplay videos={videosRelated} direction="column" />
      </Box>
      </Stack>
